...
try {
    ...
    $id_afiliado_post = (int)($_POST['id_afiliado'] ?? 0);
    $id_titular_post  = (int)($_POST['id_titular'] ?? 0);
...
// Si vino id_afiliado, es un beneficiario (menor / etc.)
if ($id_afiliado_post > 0) {
    $sql_af = "SELECT id, id_titular, tipo_beneficiario
               FROM afiliados
               WHERE id = ? AND estado = 'activo'";
    $stmt = $conn->prepare($sql_af);
    $stmt->bind_param('i', $id_afiliado_post);
    $stmt->execute();
    $stmt->bind_result($id_af, $id_tit, $tipo_benef);

    if ($stmt->fetch()) {
        $tipo_entidad = $tipo_benef; // ej: 'hijo menor'
        $id_afiliado = $id_af;       // el menor
        $id_titular  = $id_tit;      // el titular (en afiliados)

        // Validación simple: si es menor, debe tener titular
        ...
        $stmt->close();
        echo json_encode(['ok' => false, 'msg' => 'Afiliado no encontrado']);
        exit;
    }
    $stmt->close();
} else {
    // Si NO vino id_afiliado, validamos que exista el paciente
    $sql_pac = "SELECT id_paciente FROM pacientes WHERE id_paciente = ?";
