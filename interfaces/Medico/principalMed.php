...
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    /* SweetAlert2 siempre arriba de tu modal */
.swal2-container{
...
<body>
      ...
      <section class="card" aria-label="Accesos rápidos">
        ...
        <div class="actions">
          <article class="action" role="button" tabindex="0"
            ...
            <div>
              <h3 style="margin:0 0 6px;" color="white">Gestionar Agenda</h3>
  ...
  <div class="modal" id="modalConfirmarPassword">
    <div class="modal-content">
      <div class="modal-header">
        <h3><i class="fa-solid fa-shield-halved"></i> Generar Firma Digital</h3>
  ...
  async function swConfirm(title, text){
  ...
  if (!password) {
    await swInfo('Debe ingresar su contraseña.');
    return;
  }

  const confirmado = await swConfirm(
    'Confirmación requerida',
    '¿Confirmás generar tu firma digital? Este proceso se ejecuta una sola vez.'
  );
  ...
  try {
    ...
    if (data.ok) {
      await swOk(data.msg || 'Claves generadas.');
      cerrar();
      await verificarEstadoClaves(); // Recargar estado
    } else {
      await swErr(data.msg || 'Error al generar claves.');
      formGenerar.style.display = 'block';
      loadingClaves.style.display = 'none';
    }
  } catch (error) {
    console.error('Error:', error);
    await swErr('Error de conexión al generar las claves.');
    formGenerar.style.display = 'block';
    loadingClaves.style.display = 'none';
  }
};
