...
(() => {
  ...
  e.preventDefault(); // siempre frenamos el submit y lo manejamos nosotros

  if (!fechaHidden.value || !horaHidden.value) {
    await swInfo('Debe seleccionar una fecha y hora del calendario.');
    return;
  }

  const r = await swConfirm('¿Confirmás enviar la derivación al médico seleccionado?');
  if (r.isConfirmed) {
    e.target.submit(); // ahora sí, submit real del form
  }
});
