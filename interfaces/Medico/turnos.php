...
$stmt = $cn->prepare("
    ...
    FROM pacientes p
    JOIN usuarios u ON u.id_usuario = p.id_usuario
    WHERE p.id_paciente = (SELECT id_paciente FROM turnos WHERE id_turno = ?)
");
...
$datos_notificacion = [
    ...
    'tipo'        => 'Turno Médico',  // ajustá según corresponda
    'detalle'     => $motivo ?: 'Cancelación de turno',
    'fecha'       => $info['fecha'],
    'hora'        => $info['hora'],
    'profesional' => "Médico",   // nombre del médico
    'direccion'   => 'Acellaneda',
];
...
$datos_notificacion = [
    ...
    'detalle'     => 'Reprogramación de turno',
    'fecha'       => $nf,  // nueva fecha
    'hora'        => $nh,  // nueva hora
    'profesional' => $displayRight,
    'direccion'   => 'Avellaneda',
    'fecha_anterior' => $info['fecha'],  // opcional: para mostrar fecha vieja
    'hora_anterior'  => $info['hora'],
];

// Enviar notificación
enviarNotificacion('reprogramar_turno', $datos_notificacion);
...
(() => {
  ...
  function swErr(msg){
    return Swal.fire({
      icon: 'error',
      title: 'Atención',
      text: msg || 'No se pudo completar la operación.',
      confirmButtonText: 'Aceptar',
  ...
  function swConfirm({title, text, confirmText='Sí', cancelText='No'}){
    return Swal.fire({
      icon: 'warning',
  ...
  function mostrarDetallePendiente(t){
    ...
    document.getElementById('btnAceptarDeriv').onclick = async () => {
      if (requiereAgenda) {
        ...
        if(!f||!h){ await swInfo('Elegí fecha y horario.'); return; }
...
if (d.ok) await swOk(d.msg);
else await swErr(d.msg);
...
if(d.ok){ modal.style.display='none'; switchTab('cancelado'); }
 ...
 function abrirReprogramar(t){
  ...
  document.getElementById('btnGuardarRep').onclick = async () => {
    if(!fechaSeleccionada || !horaSeleccionada){ 
      await swInfo('Seleccioná una fecha y horario del calendario.');
      return; 
    }
