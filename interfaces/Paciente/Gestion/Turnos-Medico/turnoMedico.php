...
<?php endif; ?>
  ...
  function confirmarTurno(fecha, horaInicio, idMedico) {
    ...
    // Nombre del titular desde PHP para usar si no hay selector o elige "Para mí"
    ...
    if (selector && selector.selectedIndex >= 0) {
        ...
        if (selectedOption.value.startsWith('p-')) {
            ...
            // Limpiamos el texto del <option> para que solo quede el nombre
            nombreBeneficiario = selectedOption.text.split(' -')[0].split(' (')[0].trim();
        }
    } else {
        // Fallback si no existe el selector
        nombreBeneficiario = nombreTitular;
        idPaciente = "<?= $id_paciente ?>";
    }

    // CARTEL ESTILIZADO CON EL NOMBRE DEL PACIENTE REAL
    Swal.fire({
        title: '¿Confirmar Turno Médico?',
        html: `
            ...
            <p style="margin-top: 15px; color: #666; font-size: 0.9em;">Se enviará una notificación a su correo.</p>
        `,
        icon: 'question',
        ...
        if (result.isConfirmed) {
            ...
            let beneficiarioId = idPaciente ? `p-${idPaciente}` : `a-${idAfiliado}`;
            ...
            .then(data => {
                if (data.success) {
                    Swal.fire('¡Turno Confirmado!', data.mensaje, 'success').then(() => {
                        cerrarModalHorarios();
                        cargarCalendario();
                    });
                } else {
