...
<?php if ($es_titular && !empty($afiliados_menores)): ?>
  <div style="background: #e7f3ff; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
     <h3 style="margin-top: 0; color: black;">¿Para quién es el turno?</h3>
...
function mostrarHorarios(fecha) {
 ...
 function confirmarTurno(fecha, horaInicio, idRecurso) {
    ...
    // 🧩 Obtener beneficiario y nombre real
    const selector = document.getElementById('selector-beneficiario');
    const nombreTitular = "<?= htmlspecialchars($_SESSION['nombre'] . ' ' . $_SESSION['apellido']) ?>";
    let beneficiarioId = '';
    let nombreBeneficiario = nombreTitular;

    if (selector && selector.value) {
        beneficiarioId = selector.value;
        const selectedOption = selector.options[selector.selectedIndex];
        
        // Si es el titular, usamos el nombre de sesión, si no, limpiamos el texto del option
        if (beneficiarioId.startsWith('p-')) {
            nombreBeneficiario = nombreTitular;
        } else {
            nombreBeneficiario = selectedOption.text.split(' -')[0].split(' (')[0].trim();
        }
    } else {
    ...
    // 🎨 CARTEL ESTILIZADO (Igual al de médico pero adaptado a estudio)
    Swal.fire({
        title: '¿Confirmar Turno de Estudio?',
        html: `
            <div style="text-align: left; background: #f8f9fa; padding: 15px; border-radius: 10px; border: 1px solid #dee2e6;">
                <p style="margin: 5px 0;"><strong>🔬 Estudio:</strong> ${estudioNombre}</p>
                <p style="margin: 5px 0;"><strong>👤 Paciente:</strong> ${nombreBeneficiario}</p>
                <p style="margin: 5px 0;"><strong>📅 Fecha:</strong> ${fecha}</p>
                <p style="margin: 5px 0;"><strong>🕐 Hora:</strong> ${horaInicio} hs</p>
                <hr style="margin: 10px 0; border: 0; border-top: 1px solid #ccc;">
                <p style="margin: 5px 0; font-size: 0.9em;"><strong>📋 Orden:</strong> ${ordenInfo.medico_nombre}</p>
                <p style="margin: 5px 0; font-size: 0.85em; color: #555;"><strong>💊 Diag:</strong> ${ordenInfo.diagnostico.substring(0, 40)}...</p>
            </div>
            <p style="margin-top: 15px; color: #666; font-size: 0.9em;">Se enviará el recordatorio a su correo.</p>
        `,
        icon: 'question',
        ...
        if (result.isConfirmed) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    Swal.fire('Error', '❌ ' + (data.error || 'No se pudo reservar'), 'error');
                }
            })
            .catch(err => {
                console.error(err);
