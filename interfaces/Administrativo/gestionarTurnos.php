...
$stmt = $conn->prepare("
    ...
    JOIN usuarios u ON u.id_usuario = p.id_usuario
    WHERE p.id_paciente = (
        SELECT id_paciente FROM turnos WHERE id_turno = ?
    )
");
$stmt->bind_param('i', $id_turno);
$stmt->execute();
...
$datos_notificacion = [
    ...
    'tipo'        => 'Turno médico',
    'detalle'     => "Cancelación de turno programado",  
...
// Enviar notificación
enviarNotificacion('cancelar_turno', $datos_notificacion);
...
$datos_notificacion = [
    ...
    'apellido'    => $paciente_data['apellido'] ?? '',
    'email'       => $paciente_data['email'] ?? '',
    'tipo'        => 'Estudio médico',          // Ajustado para estudios
    'detalle'     => "Cancelación de turno de estudio",  
    'fecha'       => $turno_data['fecha'],
    'hora'        => $turno_data['hora'],
    'profesional' => "Técnico de estudios",       // opcional, podes poner nombre real si lo tenes
    'direccion'   => "Avellaneda",   // opcional
];
...
document.getElementById('btnPrevMonth').addEventListener('click', function() {
  ...
  // Verificar si el mes anterior sería anterior al mes actual
  const currentDate = new Date();
  const currentTotalMonths = currentDate.getFullYear() * 12 + currentDate.getMonth();
  const prevTotalMonths = prevYear * 12 + (adjustedPrevMonth - 1);
  
  if (prevTotalMonths < currentTotalMonths) {
    return;  // No permitir ir más atrás del mes actual
  }
  
  currentMonth = adjustedPrevMonth;
...
document.getElementById('btnNextMonth').addEventListener('click', function() {
  ...
  // Calcular diferencia de meses desde el mes actual
  const currentDate = new Date();
  const currentTotalMonths = currentDate.getFullYear() * 12 + currentDate.getMonth();
  const nextTotalMonths = nextYear * 12 + (adjustedNextMonth - 1);
  const monthsDiff = nextTotalMonths - currentTotalMonths;
...
async function renderCalendar() {
  ...
  let estadosMap = {};
  try {
    const response = await fetch(`api/agenda_estado.php?id_medico=${currentMedico}&mes=${currentMonth}&anio=${currentYear}`);
...
async function selectDate(date) {
  ...
  // Actualiza la clase 'selected' en los días existentes sin re-renderizar todo el calendario
  ...
  try {
    ...
    const data = await response.json();
    if (data.error) {
      // Usamos la función personalizada para mostrar errores
...
if (selectPacienteEstudio) {
  selectPacienteEstudio.addEventListener('change', async function() {
    ...
    try {
      ...
      if (data.error) {
        // Mostrar error sin bloquear con alert
        mostrarAlerta('error', data.error);
        return;
      }
      ...
      if (ordenesDisponibles.length === 0) {
        ...
        // Usamos mostrarAlerta para mostrar el mensaje sin bloquear
      ...
      console.error('Error:', error);
      // Usamos mostrarAlerta para mostrar el error sin bloquear
...
document.getElementById('btnPrevMonthEstudio').addEventListener('click', function() {
  const prevMonth = currentMonthEstudio - 1;
  const prevYear = prevMonth < 1 ? currentYearEstudio - 1 : currentYearEstudio;
  const adjustedPrevMonth = prevMonth < 1 ? 12 : prevMonth;
  ...
  currentMonthEstudio = adjustedPrevMonth;
  currentYearEstudio = prevYear;
  loadCalendarEstudio();
});

document.getElementById('btnNextMonthEstudio').addEventListener('click', function() {
  const nextMonth = currentMonthEstudio + 1;
  const nextYear = nextMonth > 12 ? currentYearEstudio + 1 : currentYearEstudio;
  const adjustedNextMonth = nextMonth > 12 ? 1 : nextMonth;
  ...
  if (monthsDiff >= MAX_MONTHS_ESTUDIOS) {
    return;  // No permitir ir más allá del límite
  }
  
  currentMonthEstudio = adjustedNextMonth;
  currentYearEstudio = nextYear;
  loadCalendarEstudio();
});
...
async function renderCalendarEstudio() {
  ...
  let estadosMap = {};
  try {
    const response = await fetch(`api/agenda_estado_estudios.php?id_estudio=${currentEstudio}&mes=${currentMonthEstudio}&anio=${currentYearEstudio}`);
    const data = await response.json();
    if (Array.isArray(data)) {
      data.forEach(d => {
        estadosMap[d.dia] = {
...
async function selectDateEstudio(date) {
  ...
  try {
    ...
    const data = await response.json();
    if (data.error) {
      // Usamos la función mostrarAlerta para mostrar el error
      mostrarAlerta('error', data.error);
      return;
    }
    const slots = data.slots || [];
    ...
    console.error('Error:', error);
    // Usamos mostrarAlerta para mostrar el error de manera no bloqueante
    mostrarAlerta('error', 'Error al cargar horarios');
  } finally {
...
function selectSlotEstudio(hora, id_tecnico, id_recurso) {
  ...
  if (!selectedDateEstudio || !/^\d{4}-\d{2}-\d{2}$/.test(selectedDateEstudio)) {
    // Reemplazamos la alerta tradicional con una alerta no bloqueante
  ...
  // Remover la clase 'selected' de todos los botones de horario
  document.querySelectorAll('#slotsGridEstudio .slot-btn').forEach(btn => {
    btn.classList.remove('selected');
    // Agregar la clase 'selected' al botón de la hora seleccionada
