...
try {
    ...
    // Obtener id_estado de 'confirmado' y 'reprogramado'
$resEstados = $conn->query("SELECT id_estado, nombre_estado FROM estados WHERE nombre_estado IN ('Confirmado', 'Reprogramado')");
$estado_confirmado = null;
$estado_reprogramado = null;

while ($row = $resEstados->fetch_assoc()) {
    if ($row['nombre_estado'] === 'Confirmado') $estado_confirmado = intval($row['id_estado']);
    if ($row['nombre_estado'] === 'Reprogramado') $estado_reprogramado = intval($row['id_estado']);
}


if ($estado_confirmado === null || $estado_reprogramado === null) {
    http_response_code(500);
    echo json_encode(['error' => 'Los estados "confirmado" y/o "reprogramado" no existen en la base de datos.']);
    exit;
}


// Verificar duplicado del turno (cualquier fecha)
$stmtDup = $conn->prepare("
    ...
    WHERE id_paciente = ?
      ...
      AND id_estado IN (?, ?)
      AND (
            (id_afiliado IS NULL AND ? IS NULL)
         OR (id_afiliado = ?)
      )
");
...
if ($cntDup > 0) {
    ...
    echo json_encode([
        'error' => $afiliado_id
            ? 'El afiliado ya tiene un turno confirmado o reprogramado con este médico.'
            : 'Usted ya tiene un turno confirmado o reprogramado con este médico.'
    ]);
    exit;
}
