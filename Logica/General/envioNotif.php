...
function enviarNotificacion(string $accion, array $datos): bool
{
    ...
    switch ($accion) {
        ...
        case 'reprogramar_turno':
            $mail->Subject = 'Turno reprogramado 🔄';
            $mail->Body    = plantillaReprogramarTurno($datos);
            break;
        default:
...
function plantillaReprogramarTurno($d)
{
    $nombre = htmlspecialchars($d['nombre'] ?? '');
    $apellido = htmlspecialchars($d['apellido'] ?? '');
    $fecha = htmlspecialchars($d['fecha'] ?? '');
    $hora = htmlspecialchars($d['hora'] ?? '');
    $fecha_ant = htmlspecialchars($d['fecha_anterior'] ?? '');
    $hora_ant = htmlspecialchars($d['hora_anterior'] ?? '');
    $tipo = htmlspecialchars($d['tipo'] ?? '');
    $detalle = htmlspecialchars($d['detalle'] ?? '');
    $profesional = htmlspecialchars($d['profesional'] ?? '');
    $direccion = htmlspecialchars($d['direccion'] ?? '');
    ...
    return "
        <div style='font-family: Arial, sans-serif; background:#f9f9f9; padding:20px;'>
            <div style='max-width:600px; margin:auto; background:#fff; border-radius:8px; box-shadow:0 2px 6px rgba(0,0,0,0.1);'>
                <div style='background:#FF9800; padding:20px; text-align:center; color:#fff;'>
                    <h2 style='margin:0;'>Turno Reprogramado 🔄</h2>
                </div>
                <div style='padding:20px; color:#333;'>
                    <p>Estimado/a <b>{$nombre} {$apellido}</b>,</p>
                    <p>Su turno ha sido <b>reprogramado</b>:</p>
                    
                    <div style='background:#ffebee; padding:12px; border-radius:6px; margin:10px 0;'>
                        <p style='margin:0;'><b>Fecha anterior:</b> {$fecha_ant} {$hora_ant}</p>
                    </div>
                    
                    <div style='background:#e8f5e9; padding:12px; border-radius:6px; margin:10px 0;'>
                        <p style='margin:0;'><b>Nueva fecha:</b> {$fecha} {$hora}</p>
                    </div>
                    ...
                    <p style='margin-top:20px;'>Por favor, confirme su asistencia.</p>
                </div>
                <div style='background:#f5f5f5; padding:10px; text-align:center; font-size:12px; color:#777;'>
                    © " . date('Y') . " Sistema de Gestión Turnos – Mensaje automático.
                </div>
